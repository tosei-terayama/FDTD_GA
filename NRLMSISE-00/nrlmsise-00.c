...
__inline__ double zeta(double zz, double zl) {
...
/*    Eq. A24d */
__inline__ double g0(double a, double *p) {
	return (a - 4.0 + (p[25] - 1.0) * (a - 4.0 + (exp(-sqrt(p[24]*p[24]) * (a - 4.0)) - 1.0) / sqrt(p[24]*p[24])));
}

/*    Eq. A24c */
__inline__ double sumex(double ex) {
	return (1.0 + (1.0 - pow(ex,19.0)) / (1.0 - ex) * pow(ex,0.5));
}

/*    Eq. A24a */
__inline__ double sg0(double ex, double *p, double *ap) {
