...
C
      subroutine iri(xlat, xlon, iy, imd, hour, vbeg, vend, vstp, outf2)
c        xlat = 35
c        xlon = 140
c        iy = 2016
c        imd = 818
c     hour = 12 (UT)
c     vbeg: 開始高度、vend: 終了高度、vstp: 高さステップ
c        vbeg = 60
c        vend = 1000
c        vstp = 10
...
c
c     1       print *,'jmag(=0/1,geog/geom),lati/deg,long/deg'
 1    continue
c     read(5,*) jm,xlat,xlon
c     jm: 緯度経度を「地理緯度」(0)または、「磁気緯度」(1)で指定する
c     xlat: 緯度、xlon: 経度
      jm = 0
c        xlat = 35
c        xlon = 140
...
c        print *,'upper height [km] for TEC integration (0 for no TEC)'
c     read(5,*) htec_max
      htec_max = 0
...
c
 ...
 3295 format('#', A16,
     &     ' option is used for the F1 occurrence probability')
...
c
 ...
 8192 FORMAT('#'/'#-'/'#',2X,A5,6X,'PEAK ALTITUDES IN KM',8X,
     &     
     &     'SITIES IN cm-3  TEC top/%'/3X,A4,
     &     '    hmF2  hmF1   hmE   ',
     &     'hmD      NmF2   NmF1    NmE    NmD  1E16m-2')
 8194 FORMAT(/'-'/2X,A5,3X,'M3000   B0',3X,'B1   E-VALLEY',7X,
     &     'PLASMA ','FREQUENCIES / MHz'/3X,A4,11X,
     &     'km       W/km ',
     &     ' Depth',5X,'foF2   foF1   foE   foD')
 8193 FORMAT('#'/'#-'/'#',1X,A5,
     &     ' ELECTRON DENSITY   TEMPERATURES ',
     &     5X,'ION PERCENTAGES[%]*10',4X,'1E16m-2'/'#',
     &     2X,A4,' Ne/cm-3 Ne/',
     &     'NmF2 Tn/K  Ti/K  Te/K  O+  N+  ',
     &     'H+ He+ O2+ NO+ Clust TEC t/%')
 9193 FORMAT(/'-'/1X,A5,' ELECTRON DENSITY   TEMPERATURES ',
     &     4X,'ION DENSITIES[cm-3]/100',4x,
     &     '1E16m-2'/2X,A4,' Ne/cm-3 Ne/',
...
c     2357    print *,'Enter 0 to exit or 1 to generate another profile?'
 2357 continue
c     read(5,*) icontinue
      icontinue = 0
