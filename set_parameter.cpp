...
void set_parameter(perturbation* P_info, bool* chrom){
    ...
    for(int i = 0; i < Num_Individual; i++){
        ...
        P_info[i].set_phi( b2d(bit3, Nbit_phi, param3_min, param3_step) );
        count += Nbit_phi;

        for(int j = 0; j < Nbit_sigr; j++){
            bit4[j] = chrom[i*Nbit_total + j + count];
        }
        P_info[i].set_sigr( b2d(bit4, Nbit_sigr, param4_min, param4_step) );
        count += Nbit_sigr;

        for(int j = 0; j < Nbit_sigh; j++){
            bit5[j] = chrom[i*Nbit_total + j + count];
        }
        P_info[i].set_sigh( b2d(bit5, Nbit_sigh, param5_min, param5_step) );
        count = 0;
